...
var useMotionStyles = function (values, styleProp, transformValues) {
    ...
    for (var key in styleProp) {
        ...
        else if (styler.isTransformProp(key) || isTransformOriginProp(key)) {
...
function snapshotStyle(element, valueHandlers) {
    ...
    heyListen.warning(computedStyle.display !== "inline", "Magic components can't be display: inline, as inline elements don't accept a transform. Try inline-block instead.");
    return style;
}
...
function calcOrigin(before, after) {
    ...
    if (beforeSize > afterSize) {
        origin = popcorn.progress(before.min, before.max - afterSize, after.min);
    }
    else if (afterSize > beforeSize) {
        origin = popcorn.progress(after.min, after.max - beforeSize, before.min);
    }
...
function calcTranslate(before, after, origin) {
    var beforePoint = popcorn.mix(before.min, before.max, origin);
    var afterPoint = popcorn.mix(after.min, after.max, origin);
...
function tweenAxis(axis, target, prev, next, p) {
    target[axis].min = popcorn.mix(prev[axis].min, next[axis].min, p);
    target[axis].max = popcorn.mix(prev[axis].max, next[axis].max, p);
}
/**
...
function fixTransparentRGB(a, b) {
    ...
    var fixed = isBlack && isTransparent ? tslib.__assign(tslib.__assign({}, b), { alpha: 0 }) : a;
    return styleValueTypes.rgba.transform(fixed);
}
function fixTransparentRGBPair(origin, target) {
    var parsedOrigin = styleValueTypes.rgba.parse(origin);
    var parsedTarget = styleValueTypes.rgba.parse(target);
    return [
...
var useMotionContext = function (parentContext, controls, values, isStatic, _a) {
    ...
    var layoutDeltas = React.useRef(tslib.__spreadArrays((parentContext.layoutDeltas || []), [
        layoutDelta,
    ]));
...
var NativeElement = /** @class */ (function () {
    ...
    NativeElement.prototype.mount = function (element) {
        var _this = this;
        heyListen.invariant(element instanceof Element, "No ref found. Ensure components created with motion.custom forward refs using React.forwardRef");
        this.hasMounted = true;
        this.current = element;
        this.styler = styler__default(element, this.stylerConfig);
    ...
    NativeElement.prototype.getStyle = function (key) {
        heyListen.invariant(this.hasMounted, "Attempting to read styles of an unmounted element");
...
var Gestures = {
    ...
    Component: makeRenderlessComponent(function (_a) {
        var nativeElement = _a.nativeElement, props = tslib.__rest(_a, ["nativeElement"]);
...
transformPagePoint) {
    heyListen.invariant(constraintsElement !== null && draggableElement !== null, "If `dragConstraints` is set as a React ref, that ref must be passed to another component's `ref` prop.");
...
var Drag = {
    ...
    Component: makeRenderlessComponent(function (_a) {
        var nativeElement = _a.nativeElement, values = _a.values, controls = _a.controls, props = tslib.__rest(_a, ["nativeElement", "values", "controls"]);
...
function convertSingleRadius(value, axis) {
    ...
    if (styleValueTypes.px.test(value)) {
        return parsed;
    }
    else {
...
var singleBorderRadius = function (key) { return ({
    ...
    createUpdater: function (values, origin, target, current, delta, treeScale, originBox, targetBox) {
        ...
        return function (p) {
            var vx = popcorn.mix(originAsPixels.x, targetAsPixels.x, p);
            var vy = popcorn.mix(originAsPixels.y, targetAsPixels.y, p);
...
var defaultMagicValues = {
    ...
    boxShadow: {
        createUpdater: function (values, origin, target, current, delta, treeScale) {
            ...
            var currentShadow = tslib.__spreadArrays(originShadow);
            var mixShadowColor = popcorn.mixColor(originShadow[0], currentShadow[0]);
            var shadowTemplate = styleValueTypes.complex.createTransformer(target !== "none" ? target : origin);
            var dx = delta.x;
            var dy = delta.y;
            ...
            return function (p) {
                ...
                currentShadow[1] = popcorn.mix(originShadow[1], targetShadow[1], p); // x
                currentShadow[2] = popcorn.mix(originShadow[2], targetShadow[2], p); // y
                currentShadow[3] = popcorn.mix(originShadow[3], targetShadow[3], p); // blur
                currentShadow[4] = popcorn.mix(originShadow[4], targetShadow[4], p); // spread
                ...
                var averageXYScale = popcorn.mix(dx.scale, dy.scale, 0.5);
                var averageTreeXTScale = popcorn.mix(treeScale.x, treeScale.y, 0.5);
                currentShadow[3] =
...
function getAnimatableShadow(shadow, fallback) {
    if (shadow === "none") {
        shadow = styleValueTypes.complex.getAnimatableNone(fallback);
    }
    return styleValueTypes.complex.parse(shadow);
}
...
var SharedLayoutContextProvider = function (props) {
    ...
    var _b = React.useContext(MotionPluginContext), autoValues = _b.autoValues, transformPagePoint = _b.transformPagePoint;
...
var Auto = /** @class */ (function (_super) {
    ...
    function Auto(props) {
        ...
        _this.supportedAutoValues = tslib.__assign(tslib.__assign({}, defaultMagicValues), autoValues);
    ...
    Auto.prototype.startAnimation = function (_a) {
        ...
        if (_a === void 0) { _a = {}; }
        var _b, _c;
        var origin = _a.origin, target = _a.target, visibilityAction = _a.visibilityAction, opts = tslib.__rest(_a, ["origin", "target", "visibilityAction"]);
    ...
    Auto.prototype.startLayoutAnimation = function (opts) {
        ...
        if (animate !== false) {
            ...
            if (!dragControls || !dragControls.isDragging) {
                animation = startAnimation("progress", this.progress, progressTarget, tslib.__assign(tslib.__assign({}, (opts.transition || transition)), { restDelta: 1, restSpeed: 10 })).then(function () { return _this.safeToRemove(); });
            }
            else {
        ...
        var scheduleUpdate = function () { return sync__default.update(frame, false, true); };
    ...
    Auto.prototype.updateRotate = function (p, rotate) {
        var target = popcorn.mix(this.visualOrigin.style.rotate, this.visualTarget.style.rotate, p);
        rotate.set(target);
    };
...
function createDomMotionConfig(ComponentToRender) {
    ...
    return {
        renderComponent: function (nativeElement, style, values, props, isStatic) {
            ...
            return React.createElement(ComponentToRender, tslib.__assign(tslib.__assign(tslib.__assign({}, forwardedProps), { ref: nativeElement.ref }), staticVisualStyles));
        },
        /**
        ...
        loadFeatures: function (nativeElement, values, props, context, parentContext, controls, inherit, plugins) {
            ...
            for (var i = 0; i < numFeatures; i++) {
                ...
                if (shouldRender(props, parentContext)) {
                    activeComponents.push(React.createElement(Component, tslib.__assign({ key: key }, props, { localContext: context, parentContext: parentContext, values: values, controls: controls, nativeElement: nativeElement })));
                }
            }
...
var PresenceChild = function (_a) {
    ...
    return (React.createElement(PresenceContext.Provider, { value: tslib.__assign(tslib.__assign({ id: id }, context), { register: register }) }, children));
};
...
var crossfadeIn = function (_origin, target, p) {
    return popcorn.mix(0, target, easeCrossfadeIn(p));
};
var crossfadeOut = function (origin, _target, p) {
    return popcorn.mix(origin, 0, easeCrossfadeOut(p));
};
...
var LayoutStack = /** @class */ (function () {
    ...
    LayoutStack.prototype.shouldStackAnimate = function () {
        ...
        return this.lead && ((_a = this.lead) === null || _a === void 0 ? void 0 : _a.isPresent())
            ? ((_c = (_b = this.lead) === null || _b === void 0 ? void 0 : _b.props) === null || _c === void 0 ? void 0 : _c._shouldAnimate) === true
            : this.follow && ((_d = this.follow) === null || _d === void 0 ? void 0 : _d.props._shouldAnimate) === true;
    };
        ...
        _this.state = {
            ...
            forceRender: function () {
                if (!_this.isAnimating) {
                    _this.shouldRerender = false;
                    _this.setState(tslib.__assign({}, _this.state));
                }
                else {
        ...
        var handler = {
            snapshotTarget: function (child) {
                ...
                isAutoAnimate(child) ||
                    ...
                    (((_b = (_a = stack) === null || _a === void 0 ? void 0 : _a.lead) === null || _b === void 0 ? void 0 : _b.isPresent()) && ((_c = stack) === null || _c === void 0 ? void 0 : _c.shouldStackAnimate()))) {
            ...
            startAnimation: function (child) {
                ...
                var animation = child.startAnimation(tslib.__assign(tslib.__assign(tslib.__assign({}, options), config), { shouldAnimate: shouldAnimate }));
                if (!animation)
                    return;
